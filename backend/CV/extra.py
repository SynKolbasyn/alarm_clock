...
def get_named_edges(keypoints_with_scores, height, width, keypoint_threshold=0.11):
    keypoints_all = []
    keypoint_edges_all = []
    edge_colors = []
    num_instances, _, _, _ = keypoints_with_scores.shape
    ...
    for idx in range(num_instances):
        ...
        for edge_pair, color in KEYPOINT_EDGE_INDS_TO_COLOR.items():
            if (kpts_scores[edge_pair[0]] > keypoint_threshold and
                ...
                edge_colors.append(color)
                edges_with_names[
                    f'{KEYPOINT_DICT_INVERTED[edge_pair[0]]}-{KEYPOINT_DICT_INVERTED[edge_pair[1]]}'] = line_seg
